...
# write code that DETECTS RIGHT TRIANGLES
...
#       0. take in a initial amount of money, an interest rate (in between 0 and 1), and the number of years
...
#       3. print: "After _ years, you will have $_" where the first _ is the number of years and the second _ is the resulting value A

# write code that FINDS THE AREA OF A PARALLELOGRAM
#       0. take in 3 numbers through 3 separate calls to input(...) (the height of the parallelogram, the length of one side, the length of the other side)
#       1. convert these to floats
#       2. compute the area of the parallelogram formed by those lengths
#       3. print the resulting area

# write code that FINDS A POINT ON A QUADRATIC FUNCTION
#       0. take in 3 numbers through 3 separate calls to input(...) (a, b, c) (where each are the respective constants in a quadratic function f(x) =a^2x + bx + c)
#       1. convert these 3 numbers to floats
#       2. take in 1 number through another call to input(...) which will be x and convert it to a float
#       3. now compute f(x) and print the result
